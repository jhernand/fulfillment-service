...
`chart/templates/grpc-server/authconfig.yaml`. The configuration includes:
...
`chart/templates/grpc-server/authconfig.yaml`. To customize which claims are used (for
...
- Template: `chart/templates/grpc-server/authconfig.yaml`
...
```bash
helm upgrade fulfillment-service chart -n osac
```
...
- [Service Chart README](chart/README.md)
