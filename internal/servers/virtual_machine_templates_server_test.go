...
var _ = Describe("Virtual machine templates server", func() {
	...
	Describe("Behaviour", func() {
		var server *VirtualMachineTemplatesServer
		...
		It("Creates object", func() {
			response, err := server.Create(ctx, ffv1.VirtualMachineTemplatesCreateRequest_builder{
				Object: ffv1.VirtualMachineTemplate_builder{
					Title:       "My title",
		...
		It("Creates object with parameters", func() {
			response, err := server.Create(ctx, ffv1.VirtualMachineTemplatesCreateRequest_builder{
				Object: ffv1.VirtualMachineTemplate_builder{
					Title:       "My title",
					Description: "My description.",
					Parameters: []*ffv1.VirtualMachineTemplateParameterDefinition{
						ffv1.VirtualMachineTemplateParameterDefinition_builder{
							Name:        "cpu_count",
							Title:       "CPU Count",
		...
		It("List objects", func() {
			...
			const count = 10
			for i := range count {
				_, err := server.Create(ctx, ffv1.VirtualMachineTemplatesCreateRequest_builder{
					Object: ffv1.VirtualMachineTemplate_builder{
			...
			// List the objects:
			response, err := server.List(ctx, ffv1.VirtualMachineTemplatesListRequest_builder{}.Build())
			Expect(err).ToNot(HaveOccurred())
			Expect(response).ToNot(BeNil())
		...
		It("List objects with limit", func() {
			...
			response, err := server.List(ctx, ffv1.VirtualMachineTemplatesListRequest_builder{
				Limit: proto.Int32(5),
			}.Build())
			Expect(err).ToNot(HaveOccurred())
			Expect(response).ToNot(BeNil())
		...
		It("Gets object", func() {
			// Create an object:
			createResponse, err := server.Create(ctx, ffv1.VirtualMachineTemplatesCreateRequest_builder{
				Object: ffv1.VirtualMachineTemplate_builder{
					Title:       "My title",
					Description: "My description.",
				}.Build(),
			...
			// Get the object:
			getResponse, err := server.Get(ctx, ffv1.VirtualMachineTemplatesGetRequest_builder{
				Id: id,
			}.Build())
			Expect(err).ToNot(HaveOccurred())
		...
		It("Updates object", func() {
			...
			// Update the object:
			updateResponse, err := server.Update(ctx, ffv1.VirtualMachineTemplatesUpdateRequest_builder{
				Object: ffv1.VirtualMachineTemplate_builder{
					Id:          id,
		...
		It("Deletes object", func() {
			...
			// Delete the object:
			deleteResponse, err := server.Delete(ctx, ffv1.VirtualMachineTemplatesDeleteRequest_builder{
				Id: id,
			}.Build())
			Expect(err).ToNot(HaveOccurred())
		...
		It("Handles empty object in create request", func() {
			...
			response, err := server.Create(ctx, ffv1.VirtualMachineTemplatesCreateRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
		...
		It("Handles empty object in update request", func() {
			...
			response, err := server.Update(ctx, ffv1.VirtualMachineTemplatesUpdateRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
		...
		It("Handles empty ID in get request", func() {
			...
			response, err := server.Get(ctx, ffv1.VirtualMachineTemplatesGetRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
		...
		It("Handles empty ID in delete request", func() {
			...
			response, err := server.Delete(ctx, ffv1.VirtualMachineTemplatesDeleteRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
		...
		It("Handles update without ID", func() {
			...
			response, err := server.Update(ctx, ffv1.VirtualMachineTemplatesUpdateRequest_builder{
				Object: ffv1.VirtualMachineTemplate_builder{
					Title:       "My title",
					Description: "My description.",
				}.Build(),
