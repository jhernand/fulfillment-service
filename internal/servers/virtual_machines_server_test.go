...
var _ = Describe("Virtual machines server", func() {
	...
	Describe("Behaviour", func() {
		...
		It("Creates object", func() {
			...
			response, err := server.Create(ctx, ffv1.VirtualMachinesCreateRequest_builder{
				Object: ffv1.VirtualMachine_builder{
					Spec: ffv1.VirtualMachineSpec_builder{
						Template:           "general.small",
						TemplateParameters: templateParams,
					}.Build(),
					Status: ffv1.VirtualMachineStatus_builder{
						State: ffv1.VirtualMachineState_VIRTUAL_MACHINE_STATE_PROGRESSING,
					}.Build(),
				}.Build(),
			}.Build())
			Expect(err).ToNot(HaveOccurred())
			...
			Expect(object.GetStatus().GetState()).To(Equal(ffv1.VirtualMachineState_VIRTUAL_MACHINE_STATE_PROGRESSING))
		})
		...
		It("List objects", func() {
			...
			for i := range count {
				...
				_, err := server.Create(ctx, ffv1.VirtualMachinesCreateRequest_builder{
					Object: ffv1.VirtualMachine_builder{
						Spec: ffv1.VirtualMachineSpec_builder{
							Template: templateID,
						}.Build(),
			...
			// List the objects:
			response, err := server.List(ctx, ffv1.VirtualMachinesListRequest_builder{}.Build())
			Expect(err).ToNot(HaveOccurred())
			Expect(response).ToNot(BeNil())
		...
		It("List objects with limit", func() {
			...
			response, err := server.List(ctx, ffv1.VirtualMachinesListRequest_builder{
				Limit: proto.Int32(5),
			}.Build())
			Expect(err).ToNot(HaveOccurred())
			Expect(response).ToNot(BeNil())
		...
		It("Gets object", func() {
			...
			// Create an object:
			createResponse, err := server.Create(ctx, ffv1.VirtualMachinesCreateRequest_builder{
				Object: ffv1.VirtualMachine_builder{
					Spec: ffv1.VirtualMachineSpec_builder{
						Template: "general.small",
					}.Build(),
			...
			// Get the object:
			getResponse, err := server.Get(ctx, ffv1.VirtualMachinesGetRequest_builder{
				Id: id,
			}.Build())
			Expect(err).ToNot(HaveOccurred())
		...
		It("Updates object", func() {
			...
			// Update the object:
			updateResponse, err := server.Update(ctx, ffv1.VirtualMachinesUpdateRequest_builder{
				Object: ffv1.VirtualMachine_builder{
					Id: id,
					Spec: ffv1.VirtualMachineSpec_builder{
					...
					Status: ffv1.VirtualMachineStatus_builder{
						State: ffv1.VirtualMachineState_VIRTUAL_MACHINE_STATE_READY,
					}.Build(),
				}.Build(),
			...
			Expect(object.GetStatus().GetState()).To(Equal(ffv1.VirtualMachineState_VIRTUAL_MACHINE_STATE_READY))
		})
		...
		It("Deletes object", func() {
			...
			// Delete the object:
			deleteResponse, err := server.Delete(ctx, ffv1.VirtualMachinesDeleteRequest_builder{
				Id: id,
			}.Build())
			Expect(err).ToNot(HaveOccurred())
		...
		It("Handles empty object in create request", func() {
			...
			response, err := server.Create(ctx, ffv1.VirtualMachinesCreateRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
		...
		It("Handles empty object in update request", func() {
			...
			response, err := server.Update(ctx, ffv1.VirtualMachinesUpdateRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
		...
		It("Handles empty ID in get request", func() {
			...
			response, err := server.Get(ctx, ffv1.VirtualMachinesGetRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
		...
		It("Handles empty ID in delete request", func() {
			...
			response, err := server.Delete(ctx, ffv1.VirtualMachinesDeleteRequest_builder{}.Build())
			Expect(err).To(HaveOccurred())
			Expect(response).To(BeNil())
		})
	})
