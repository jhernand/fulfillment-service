...
var _ = Describe("gRPC client", func() {
	...
	It("Retries once when the server returns the unauthenticated status code", func() {
		...
		// Thre should be two calls to get the token, and one call to invalidate it:
		token := &auth.Token{
			Access: "test-token",
	...
	It("Does not retry when the server returns a status codo other than unauthenticated", func() {
		// Create a TLS listener:
		tcpListener, err := net.Listen("tcp", "127.0.0.1:0")
		Expect(err).ToNot(HaveOccurred())
		tlsListener := tls.NewListener(tcpListener, &tls.Config{
