...
The service uses Apache Kafka for distributing change notifications between its components. You can
use any Kafka deployment as long as the client can be configured via a standard properties file. The
service passes the properties directly to the underlying librdkafka client, so any authentication
method supported by librdkafka (SASL/PLAIN, SASL/SCRAM, SASL/OAUTHBEARER, mTLS, etc.) will work.

This guide uses the Strimzi operator as an example with SASL/SCRAM-SHA-512 (username and password)
authentication.

Install the Strimzi operator using Helm:

```shell
helm upgrade strimzi-cluster-operator oci://quay.io/strimzi-helm/strimzi-kafka-operator \
--install \
--version 1.0.0 \
--namespace kafka \
--create-namespace \
...
Create a Kafka cluster with three brokers, persistent storage, and SASL/SCRAM-SHA-512 authentication
over TLS:
...
```shell
oc wait kafka/osac -n kafka --for=condition=Ready --timeout=300s
```

Create a Kafka user with SCRAM-SHA-512 credentials:
...
Wait for the user to be provisioned:

```shell
oc wait kafkauser/osac-service -n kafka --for=condition=Ready --timeout=60s
```

## Create the service secrets

The service needs three secrets in its namespace: one for the database connection, one for the
controller's OAuth credentials, and one for the Kafka client properties.
...
Create the Kafka client properties secret. This tells the gRPC server how to connect to the Kafka
broker. Extract the password generated by Strimzi for the `osac-service` user and create the
properties file:

```shell
KAFKA_PASSWORD=$(
  oc get secret -n kafka osac-service -o json |
  jq -r '.data["password"] | @base64d'
)

oc create secret generic -n osac fulfillment-kafka-properties --from-file=properties=/dev/stdin <<.
bootstrap.servers=osac-kafka-tls-bootstrap.kafka.svc.cluster.local:9093
security.protocol=sasl_ssl
sasl.mechanisms=SCRAM-SHA-512
sasl.username=osac-service
sasl.password=${KAFKA_PASSWORD}
ssl.ca.location=/etc/fulfillment-grpc-server/tls/cas/bundle.pem
.
```
