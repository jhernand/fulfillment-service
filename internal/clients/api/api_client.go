...
// ClientBuilder contains the data and logic needed to create an objec that simplifies use of the API for clients. Don't
// create instances f this directly, use the NewClient function instead.
...
func (b *ClientBuilder) Build() (result *Client, err error) {
	...
	managementClustersClient := adminv1.NewHubsClient(grpcConn)
...
func (c *Client) Hubs() adminv1.HubsClient {
	return c.hubsClient
}
...
func (c *eventsClient) Watch(ctx context.Context, request *eventsv1.EventsWatchRequest,
	...
	stream, err = c.delegate.Watch(ctx, request)
	return
}
