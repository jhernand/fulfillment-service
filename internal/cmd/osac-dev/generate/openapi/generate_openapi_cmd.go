...
func (c *runnerContext) run(cmd *cobra.Command, args []string) error {
	...
	// Prepare a temporary directory for version 3 of the OpenAPI specification:
	v2TmpDir := filepath.Join(c.tmpDir, "v2")
	err = os.MkdirAll(v2TmpDir, 0700)
	if err != nil {
		return fmt.Errorf("failed to create version 3 temporary directory: %w", err)
	}
...
func (c *runnerContext) generateModule(ctx context.Context, moduleDir string) error {
	...
	err = c.moveFile(v2TmpFile, v2OutFile)
	if err != nil {
		return err
	}
	...
	err = c.moveFile(v3TmpFile, v3OutFile)
	if err != nil {
		return err
	}
...
func (r *runnerContext) moveFile(src, dst string) error {
...
var protocGenApiV2Url = fmt.Sprintf(
	"https://github.com/grpc-ecosystem/grpc-gateway/releases/download/%[1]s/protoc-gen-openapiv2-%[1]s-linux-x86_64",
