...
// expression. If the claim matches at least oen of the regular expression then access will be allowed, otherwise it
// will be denied.
...
// grpcAclAuthzItem is the type used to read a single ACL item from a YAML document.
...
func (b *GrpcAclAuthzFuncBuilder) loadAclFile(file string, items map[string]*regexp.Regexp) error {
	// Load the YAML data:
...
func (f *grpcAclAuthzFunc) checkAcl(claims map[string]any) bool {
	for claim, pattern := range f.aclItems {
